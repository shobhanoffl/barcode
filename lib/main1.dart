...
class _MainWidgetState extends State<MainWidget> {
  ...
  Future<void> scanBarcodeNormal() async {
    ...
    try {
      ...
      for (var i = 0; i < 25; i++) {
        final cellsRow = await sheet2?.cells.row(i);
        cellsRow?.forEach((cell) {
          print(cell.toString());
          if (cell.toString() == scanBarcode) {
            setState(() {
              hasEaten = 'Yes';
            });
          }
      ...
      if (hasEaten == 'No') {
        final newRow = [scanBarcode, DateTime.now().toString()];
        sheet2?.cells.insert(newRow as List<Cell>);
        setState(() {
          hasEaten = 'Yes';
        });
      }
